#include "Player.h"

Player::Player(COORD _spawn)
{
    playerSpawn = _spawn;
    playerPosition = playerSpawn;
    playerDirection = { 0,0 };
    playerChar = 'O';
    playerBackground = ConsoleUtils::CONSOLE_COLOR::BLACK;
    playerForeground = ConsoleUtils::CONSOLE_COLOR::YELLOW;
}

void Player::Update(Map* _map, USER_INPUTS input, std::vector<Enemy>* enemies)
{
    
    COORD newPos = playerPosition;
        switch (input)
        {
        case UP:
            newPos.Y--;
            break;
        case DOWN:
            newPos.Y++;
            break;
        case RIGHT:
            newPos.X++;
            break;
        case LEFT:
            newPos.X--;
            break;
        }

        if (newPos.X < 0)
        {
            newPos.X = _map->Width - 1;
        }
        newPos.X %= _map->Width;
        if (newPos.Y < 0)
        {
            newPos.Y = _map->Height - 1;
        }
        newPos.Y %= _map->Height;

        switch (_map->GetTile(newPos.X, newPos.Y))
        {
        case Map::MAP_TILES::MAP_WALL:
            newPos.Y = playerPosition.Y;
            newPos.X = playerPosition.X;
            break;
        case Map::MAP_TILES::MAP_POINT:
            _map->points--;
            playerPoints++;
            _map->SetTile(newPos.X, newPos.Y, Map::MAP_TILES::MAP_EMPTY);
            break;
        case Map::MAP_TILES::MAP_POWERUP:
            playerPoints += 100;
            _map->SetTile(newPos.X, newPos.Y, Map::MAP_TILES::MAP_EMPTY);
            for (size_t i = 0; i < enemies->size(); i++)
            {
                enemies->at(i).PowerUpPicked();
            }
            break;
        }
        playerPosition = newPos;
}

void Player::DrawPlayer()
{
    ConsoleUtils::Console_SetPos(playerPosition);
    ConsoleUtils::Console_SetColor(playerForeground, playerBackground);
    std::cout << playerChar;
}


